...
// Start class
if ( ! class_exists( 'Agrios_Customizer' ) ) {
	class Agrios_Customizer {
		...
		public function __construct() {
			...
			$this->panels = array(
				'general' => array(
					'title' => esc_html__( 'General Options', 'agrios' )
				),
				'header' => array(
					'title' => esc_html__( 'Header', 'agrios' )
				),
				'featuredtitle' => array(
					'title' => esc_html__( 'Featured Title', 'agrios' )
				),
				'maincontent' => array(
					'title' => esc_html__( 'Main Content', 'agrios' )
				),
				'layout' => array(
					'title' => esc_html__( 'Layout', 'agrios' )
				),
				'blog' => array(
					'title' => esc_html__( 'Blog', 'agrios' )
				),
				'blogsingle' => array(
					'title' => esc_html__( 'Blog Single Post', 'agrios' )
				),
				'sidebarwidget' => array(
					'title' => esc_html__( 'Sidebar Widget', 'agrios' )
				),
				'projects' => array(
					'title' => esc_html__( 'Project Single', 'agrios' )
				),
				'services' => array(
					'title' => esc_html__( 'Service Single', 'agrios' )
				),
				'shop' => array(
					'title' => esc_html__( 'Shop', 'agrios' )
				),
				'footer' => array(
					'title' => esc_html__( 'Footer', 'agrios' )
				),
				'footerwidget' => array(
					'title' => esc_html__( 'Footer Widget', 'agrios' )
				),
				'bottombar' => array(
					'title' => esc_html__( 'Bottom Bar', 'agrios' )
				),
			);
			...
			if ( ! is_admin() || is_customize_preview() ) {
				...
				if ( is_customize_preview() && $this->enable_postMessage ) {
					...
					add_filter( 'agrios_custom_colors_css', array( $this, 'head_css' ), 999 );
				}
			}
		}
		...
		public function add_sections() {
			...
			// Apply filters
			$this->sections = apply_filters( 'agrios_customizer_sections', $this->sections );
		}
		...
		public function add_customizer_panels_sections( $wp_customize ) {
			...
			foreach( $all_panels as $id => $val ) {
				...
				if ( $is_section ) {
					...
					$wp_customize->add_section( 'agrios_'. $id, array(
						'priority' => $priority,
						'title'    => $title
					) );
				}
				...
				// Add Panel
					...
					$wp_customize->add_panel( 'agrios_'. $id, array(
						'priority' => $priority,
						'title'    => $title
					) );
				}
		...
		public function create_sections( $wp_customize ) {
			...
			foreach ( $this->sections as $section_id => $section ) {
				...
				foreach ( $settings as $setting ) {
					...
					if ( 'agrios-heading' == $control_type ) {
					...
					} elseif ( 'agrios-heading' == $control_type ) {
						$control_object = 'Agrios_Customizer_Heading_Control';
					} elseif ( 'agrios-description' == $control_type ) {
						$control_object = 'Agrios_Customizer_Description_Control';
					} elseif ( 'agrios-sortable' == $control_type ) {
						$control_object = 'Agrios_Customize_Control_Sorter';
					} elseif ( 'agrios_textarea' == $control_type ) {
						$control_object = 'Agrios_Customizer_Textarea_Control';
					} else {
		...
		public static function customize_controls_print_styles() { ?>
			 <style type="text/css">
				/* Icons */
				#accordion-panel-agrios_general > h3:before,
				#accordion-panel-agrios_typography > h3:before,
				#accordion-panel-agrios_topbar > h3:before,
				#accordion-panel-agrios_header > h3:before,
				#accordion-panel-agrios_featuredtitle > h3:before,
				#accordion-panel-agrios_maincontent > h3:before,
				#accordion-panel-agrios_layout > h3:before,
				#accordion-panel-agrios_blog > h3:before,
				#accordion-panel-agrios_blogsingle > h3:before,
				#accordion-panel-agrios_sidebarwidget > h3:before,
				#accordion-panel-agrios_projects > h3:before,
				#accordion-panel-agrios_services > h3:before,
				#accordion-panel-agrios_shop > h3:before,
				#accordion-panel-agrios_footer > h3:before,
				#accordion-panel-agrios_footerwidget > h3:before,
				#accordion-panel-agrios_bottombar > h3:before { display: inline-block; width: 20px; height: 20px; font-size: 20px; font-style: normal; vertical-align: top; margin-top: -4px; text-align: center;-webkit-font-smoothing: antialiased; -moz-osx-font-smoothing: grayscale; color: #0085ba; margin-right: 6px; font-family: "dashicons"; content: ""; }

				#accordion-panel-agrios_general > h3:before { content: "\f108" }
				#accordion-panel-agrios_typography > h3:before { content: "\f215" }
				#accordion-panel-agrios_topbar > h3:before { content: "\f480" }
				#accordion-panel-agrios_header > h3:before { content: "\f175" }
				#accordion-panel-agrios_featuredtitle > h3:before { content: "\f209" }
				#accordion-panel-agrios_maincontent > h3:before { content: "\f134" }
				#accordion-panel-agrios_layout > h3:before { content: "\f535" }
				#accordion-panel-agrios_blog > h3:before { content: "\f109" }
				#accordion-panel-agrios_blogsingle > h3:before { content: "\f109" }
				#accordion-panel-agrios_sidebarwidget > h3:before { content: "\f135" }
				#accordion-panel-agrios_projects > h3:before { content: "\f481" }
				#accordion-panel-agrios_services > h3:before { content: "\f481" }
				#accordion-panel-agrios_shop > h3:before { content: "\f174" }
				#accordion-panel-agrios_footerwidget > h3:before { content: "\f209" }
				#accordion-panel-agrios_footer > h3:before { content: "\f535" }
				#accordion-panel-agrios_footer > h3:before { content: "\f535" }
				#accordion-panel-agrios_bottombar > h3:before { content: "\f138" }
				
				/* General Tweaks */
				.customize-control-select select { min-width: 100% }

				/* Slider UI */
				li.customize-control.customize-control-agrios_slider_ui input[type=text] { width: 20%; float: left; text-align: center; }
				li.customize-control.customize-control-agrios_slider_ui .ui-slider-horizontal.agrios-slider-ui { float: right; width: 75%; height: 5px; margin-top: 10px; color: #414042; position: relative; border-radius: 3px; border: 1px solid #747474; border-bottom-color: #aeaeae; background-color: #cdcdcd; background: -webkit-linear-gradient(#aaaaaa, #cdcdcd); background: -o-linear-gradient(#aaaaaa, #cdcdcd); background: -moz-linear-gradient(#aaaaaa, #cdcdcd); background: linear-gradient(#aaaaaa, #cdcdcd); }
				li.customize-control.customize-control-agrios_slider_ui .ui-slider-horizontal .ui-slider-handle { position: absolute; z-index: 2; width: 17px; height: 17px; cursor: default; top: -7px; margin-left: -10px; border-radius: 50%; border: 1px solid #9e9e9e; -webkit-box-shadow: inset 0 1px 2px rgba(0,0,0,.07); box-shadow: inset 0 1px 2px rgba(0,0,0,.07); cursor: pointer; background-color: #f5f5f5; background: -webkit-linear-gradient(#f8f8f8, #ededed); background: -o-linear-gradient(#f8f8f8, #ededed); background: -moz-linear-gradient(#f8f8f8, #ededed); background: linear-gradient(#f8f8f8, #ededed); box-shadow: 0 2px 2px rgba(0,0,0,0.24); }

				/* Sortable */
				.agrios-sortable ul { margin-top: 10px }
				.agrios-sortable li.agrios-sortable-li { cursor: move; background: #fff; padding: 0; padding-left: 15px; height: 40px; line-height: 40px; white-space: nowrap; border: 1px solid #dfdfdf; text-overflow: ellipsis; -webkit-user-select: none; -moz-user-select: none; -ms-user-select: none; user-select: none; color: #1F1E17; margin-bottom: 5px; margin-top: 0; overflow: hidden; position: relative; }
				.agrios-sortable li.agrios-sortable-li:hover { border-color: #999; z-index: 10; }
				.agrios-sortable li.agrios-sortable-li .fa { display: block; position: absolute; top: 0; right: 0; width: 42px; height: 40px; line-height: 40px; margin: 0; text-align: center; font-size: 18px; }
				.agrios-sortable li.agrios-sortable-li .fa:after{font-family:dashicons;font-size:20px;position:absolute;left:0;top:0;content:"\f147";color:#0085ba;width:100%;text-align:center}
				.agrios-sortable li.agrios-sortable-li .fa:before{content:"";position:absolute;right:6px;top:6px;width:26px;height:26px;background-color:#fff;border:1px solid #dfdfdf}
				.agrios-sortable li.agrios-sortable-li .fa:hover:before { border-color: #0085ba; }
				.agrios-sortable li.agrios-sortable-li .agrios-hide-sortee { cursor: pointer }
				.agrios-sortable ul li:last-child { margin-bottom: 0 }
				.agrios-sortable li.agrios-hide .fa:after { color: #dfdfdf; }
				.agrios-sortable li.agrios-hide { opacity: 0.65; }

				/* Custom Heading */
				.agrios-customizer-heading { color: #23282d; display: block; padding-top: 28px; padding-bottom: 5px; border-bottom: 2px solid #ddd; font-size: 16px; font-weight: 600; }
			 </style>
		...
		public function customize_preview_init() {
			if ( $this->enable_postMessage ) {
				wp_enqueue_script( 'agrios-customize-preview',
		...
		public function loop_through_inline_css( $return = 'css' ) {
			...
			foreach ( $settings as $settings_array ) {
				...
				foreach ( $settings_array as $setting ) {
					...
					$theme_mod  = agrios_get_mod( $setting_id, $default );
					...
					if ( apply_filters( 'agrios_remove_default_mods', false ) ) {
						$get_all_mods = agrios_get_mods();
					...
					// Sanitize data
					if ( $sanitize ) {
						$theme_mod = agrios_sanitize_data( $theme_mod, $sanitize );
					} else {
						$theme_mod = $theme_mod;
					}
