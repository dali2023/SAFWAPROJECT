...
jQuery( document ).ready( function() {
	...
	function agricola_init_media_elements(cont) {
		var audio_selector = agricola_apply_filters( 'agricola_filter_mediaelements_audio_selector', 'audio:not(.inited)' ),
	...
	function agricola_tabs_ajax_content_loader(panel, page, oldPanel) {
		...
		jQuery.post(
			...
			if (rez.error !== '') {
				...
				// Append posts
				panel
					.find( '.agricola_tab_holder,.agricola_tab_content_remove' ).remove().end()
	...
	function agricola_resize_video(cont) {
		...
		if ( AGRICOLA_STORAGE['resize_tag_iframe'] ) {
			cont.find( '.video_frame iframe,iframe' ).each( function() {
				var $self = jQuery( this );
				// If item now invisible
				if ( $self.hasClass( 'trx_addons_resize' ) || $self.hasClass( 'trx_addons_noresize' ) || $self.addClass( 'agricola_resize' ).parents( 'div:hidden,section:hidden,article:hidden' ).length > 0 ) {
					return;
				}
