<div class="front_page_section front_page_section_team<?php
	$agricola_scheme = agricola_get_theme_option( 'front_page_team_scheme' );
	if ( ! empty( $agricola_scheme ) && ! agricola_is_inherit( $agricola_scheme ) ) {
		echo ' scheme_' . esc_attr( $agricola_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( agricola_get_theme_option( 'front_page_team_paddings' ) );
	if ( agricola_get_theme_option( 'front_page_team_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$agricola_css      = '';
		$agricola_bg_image = agricola_get_theme_option( 'front_page_team_bg_image' );
...
<?php
	// Add anchor
	$agricola_anchor_icon = agricola_get_theme_option( 'front_page_team_anchor_icon' );
	$agricola_anchor_text = agricola_get_theme_option( 'front_page_team_anchor_text' );
...
?>
	...
	<?php
	if ( agricola_get_theme_option( 'front_page_team_fullheight' ) ) {
	...
	?>
			...
			<?php
			$agricola_css      = '';
			$agricola_bg_mask  = agricola_get_theme_option( 'front_page_team_bg_mask' );
			$agricola_bg_color_type = agricola_get_theme_option( 'front_page_team_bg_color_type' );
			if ( 'custom' == $agricola_bg_color_type ) {
				$agricola_bg_color = agricola_get_theme_option( 'front_page_team_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_team_content_wrap content_wrap">
			<?php
			// Caption
			$agricola_caption = agricola_get_theme_option( 'front_page_team_caption' );
			if ( ! empty( $agricola_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_team_caption front_page_block_<?php echo ! empty( $agricola_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $agricola_caption, 'agricola_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$agricola_description = agricola_get_theme_option( 'front_page_team_description' );
			if ( ! empty( $agricola_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_team_description front_page_block_<?php echo ! empty( $agricola_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $agricola_description ), 'agricola_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_team_output">
				...
				if ( is_active_sidebar( 'front_page_team_widgets' ) ) {
					...
					if ( ! agricola_exists_trx_addons() ) {
						...
						agricola_customizer_need_widgets_message( 'front_page_team_caption', 'ThemeREX Addons - Team' );
					}
				}
				?>
			</div>
