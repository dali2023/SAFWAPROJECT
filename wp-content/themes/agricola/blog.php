...
if ( function_exists( 'agricola_elementor_is_preview' ) && agricola_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			agricola_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}

	// Make a new main query
	agricola_new_main_query(
		array(
			'post_type'      => agricola_get_theme_option( 'post_type' ),
			'category'       => agricola_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'agricola_filter_get_template_part', agricola_blog_archive_get_template() ) );
}
