	...
	agricola_show_post_featured(
		array(
			...
			'post_info'     => '<div class="post_header entry-header">'
									. '<div class="post_categories">' . wp_kses( agricola_get_post_categories( '' ), 'agricola_kses_content' ) . '</div>'
									. '<h6 class="post_title entry-title"><a href="' . esc_url( $agricola_link ) . '">'
										. wp_kses_data( '' == get_the_title() ? esc_html__( '- No title -', 'agricola' ) : get_the_title() )
									. '</a></h6>'
									. ( in_array( get_post_type(), array( 'post', 'attachment' ) )
											? '<div class="post_meta"><a href="' . esc_url( $agricola_link ) . '" class="post_meta_item post_date">' . wp_kses_data( agricola_get_date() ) . '</a></div>'
											: '' )
								. '</div>',
		)
	);
