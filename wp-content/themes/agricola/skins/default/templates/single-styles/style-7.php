...
if ( apply_filters( 'agricola_filter_single_post_header', is_singular( 'post' ) || is_singular( 'attachment' ) ) ) {
	...
	ob_end_clean();
	$agricola_with_featured_image = agricola_is_with_featured_image( $agricola_post_header, array( 'with_gallery' ) );
	// Post title and meta
	ob_start();
	...
	if ( strpos( $agricola_post_header, 'post_featured' ) !== false
		...
		<div class="post_header_wrap post_header_wrap_in_header post_header_wrap_style_<?php
            ...
            if ( $agricola_with_featured_image ) {
                echo ' with_featured_image' . ( false && agricola_get_theme_option( 'single_parallax' ) == 0 ? ' agricola-full-height' : '' );
            }
		?>">
			<?php
