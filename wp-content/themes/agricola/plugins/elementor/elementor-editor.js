...
jQuery( document ).ready(
	function() {
		...
		agricola_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="agricola_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.agricola_post_editor' );
						if ( link.length > 0 ) {
							agricola_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.agricola_post_editor', function(e) {
						agricola_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('agricola_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function agricola_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
