...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			agricola_create_observer( 'agricola-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				agricola_gutenberg_editor_init();
			} );
		} else {
		 	agricola_gutenberg_editor_init();
		}
	...
	function agricola_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor_type == 'site' ) {
					...
					if ( editor.$frame.length ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'agricola_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'agricola_inited' ) ) {
	...
	// Init on page load
	function agricola_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = agricola_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + AGRICOLA_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + AGRICOLA_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in AGRICOLA_STORAGE['override_classes'] ) {
				$obj.addClass( AGRICOLA_STORAGE['override_classes'][i].replace( '%s', AGRICOLA_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( agricola_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && AGRICOLA_STORAGE['sidebar_position'] != 'hide' ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'agricola_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			agricola_remove_observer( 'agricola-check-editor-styles-wrapper' );
			agricola_create_observer( 'agricola-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'agricola_inited' );
	}
} );
